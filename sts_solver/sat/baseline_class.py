"""Class-based SAT baseline solver using sequential-counter encodings."""
...
def _at_most_k(s, literals, k, prefix=""):
    n = len(literals)
    if k < 0:
        for l in literals:
            s.add(Not(l))
...
def _at_least_k(s, literals, k, prefix=""):
    if k <= 0:
        return
    n = len(literals)
    if n < k:
        s.add(BoolVal(False))
        return
    seq = [[Bool(f"{prefix}_s_{i}_{j}") for j in range(k)] for i in range(n)]
    s.add(seq[0][0] == literals[0])
    for j in range(1, k):
        s.add(Not(seq[0][j]))
    for i in range(1, n):
...
def _exactly_k(s, literals, k, prefix=""):
    _at_most_k(s, literals, k, prefix=f"{prefix}_am")
    _at_least_k(s, literals, k, prefix=f"{prefix}_al")


class SATBaselineNativeSolver(SATBaseSolver):
    def _build_model(self) -> Tuple[Solver, Dict[str, Any]]:
        ...
        # Slot constraints: exactly one home and one away in each slot
        for w in weeks:
            for p in periods:
                home_teams = [plays_home[t][w][p] for t in teams]
                away_teams = [plays_away[t][w][p] for t in teams]
                _exactly_k(s, home_teams, 1, prefix=f"h_slot_w{w}_p{p}")
                _exactly_k(s, away_teams, 1, prefix=f"a_slot_w{w}_p{p}")
                for t in teams:
                    s.add(Not(And(plays_home[t][w][p], plays_away[t][w][p])))

        # Weekly appearances: exactly one per team per week
        for t in teams:
            for w in weeks:
                weekly_app = [plays_home[t][w][p] for p in periods] + [plays_away[t][w][p] for p in periods]
                _exactly_k(s, weekly_app, 1, prefix=f"weekly_t{t}_w{w}")
        ...
        for t1, t2 in combinations(teams, 2):
            ...
            _exactly_k(s, meetings, 1, prefix=f"meet_{t1}_{t2}")

        # At most twice in the same period
        for t in teams:
            for p in periods:
                period_app = [plays_home[t][w][p] for w in weeks] + [plays_away[t][w][p] for w in weeks]
                _at_most_k(s, period_app, 2, prefix=f"period_t{t}_p{p}")

        # Symmetry: fix first week matchups
        for p in periods:
            home_team = 2 * p - 1
            away_team = 2 * p
    ...
    def get_metadata(cls) -> SolverMetadata:
        return SolverMetadata(
            ...
            description="Pure SAT baseline encoding with sequential counters",
        )
